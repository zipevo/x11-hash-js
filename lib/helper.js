 ...
 * @returns {number[]} The byte array representation of the string.
 ...
 */
module.exports.bytes2Hex = function (bytes) {
 ...
 * @returns {number[]} Result of XOR operation
 * @throws {Error} If arrays are not of the same length
 */
module.exports.xorArrays = function (arr1, arr2) {
    if (arr1.length !== arr2.length) {
        throw new Error('Arrays must be of the same length for XOR operation');
    }
    return arr1.map((value, index) => value ^ arr2[index]);
};


module.exports.int32ToBuffer = function(num) {
    const buffer = Buffer.alloc(4); // 32-bit = 4 bytes
    buffer.writeUInt32BE(num, 0); // Big-endian
    return buffer;
};
...
module.exports.int64ToBuffer = function(num) {
    const buffer = Buffer.alloc(8); // 64-bit = 8 bytes
    buffer.writeUInt32BE(Math.floor(num / 0x100000000), 0); // High 32 bits
    buffer.writeUInt32BE(num >>> 0, 4); // Low 32 bits
